...
class Unity {
public:
    static float3 ObjSpaceViewDir(float4 v, constant NodeBuffer& node, constant SCNSceneBuffer& scene)
    {
    ...
    static float4 ObjectToClipPos(float4 inPos, constant NodeBuffer& node)
    {
        return node.modelViewProjectionTransform * inPos;
    }
    
    static float3 ObjectToWorldNormal(float3 norm, constant NodeBuffer& node)
    {
        float3 worldNormal = (node.modelTransform * float4(norm, 0)).xyz;
        return normalize(worldNormal);
    }
    
    static float3 getViewRayDir(float3 vertexLocal, constant NodeBuffer& node, constant SCNSceneBuffer& scene)
    {
...
class Util {
public:
    static float normalize(short value, int minValue, int maxValue)
    {
    ...
    {
        float ndotl = max(lerp(0.0f, 1.5f, dot(normal, lightDir)), 0.5f);
...
public:
    ...
    static RayInfo getRayBack2Front(float3 vertexLocal,
                                    constant NodeBuffer& node,
                                    constant SCNSceneBuffer& scene)
    {
        RayInfo ray;
        ...
        const float3 farPos = ray.startPosition + ray.direction * ray.aabbIntersection.y - float3(0.5f);
        float4 clipPos = Unity::ObjectToClipPos(float4(farPos, 1.0f), node);
    ...
    static RaymarchInfo initRayMarch(RayInfo ray, int maxNumSteps)
    {
        ...
        raymarchInfo.numSteps = metal::clamp((int)(abs(ray.aabbIntersection.x - ray.aabbIntersection.y) / raymarchInfo.stepSize), (int)1, maxNumSteps);
        raymarchInfo.numStepsRecip = 1.0f / float(raymarchInfo.numSteps);
        return raymarchInfo;
    }

    static short getDensity(texture3d<float, access::sample> volume, float3 coord)
    {
        return short(volume.sample(sampler3d, coord).r * 4095.0);
    }
    
    static float4 getTfColour(texture2d<float, access::sample> tf, float density)
    {
        return tf.sample(sampler2d, float2(density, 0.5f));
    }
};
